...
type pinAPIClient(httpClient: HttpClient) =
    ///<summary>
    ///List all the pin objects, matching optional filters; when no filter is provided, only successful pins are returned
    ///</summary>
    ///<param name="cid">Return pin objects responsible for pinning the specified CID(s); be aware that using longer hash functions introduces further constraints on the number of CIDs that will fit under the limit of 2000 characters per URL  in browser contexts</param>
    ///<param name="name">Return pin objects with specified name (by default a case-sensitive, exact match)</param>
    ///<param name="match">Customize the text matching strategy applied when the name filter is present; exact (the default) is a case-sensitive exact match, partial matches anywhere in the name, iexact and ipartial are case-insensitive versions of the exact and partial strategies</param>
    ///<param name="status">Return pin objects for pins with the specified status</param>
    ///<param name="before">Return results created (queued) before provided timestamp</param>
    ///<param name="after">Return results created (queued) after provided timestamp</param>
    ...
    member this.GetPins
        ...
        async {
            let requestParts =
                [ if cid.IsSome then
                      RequestPart.query ("cid", cid.Value)
                  if name.IsSome then
            ...
            let! (status, content) = OpenApiHttp.getAsync httpClient "/pins" requestParts

            if status = HttpStatusCode.OK then
                return GetPins.OK(Serializer.deserialize content)
            else if status = HttpStatusCode.BadRequest then
                return GetPins.BadRequest(Serializer.deserialize content)
            else if status = HttpStatusCode.Unauthorized then
                return GetPins.Unauthorized(Serializer.deserialize content)
            else if status = HttpStatusCode.NotFound then
                return GetPins.NotFound(Serializer.deserialize content)
            else
                return GetPins.Conflict(Serializer.deserialize content)
        }

    ///<summary>
    ///Add a new pin object for the current access token
    ///</summary>
    ///<param name="body">Pin object</param>
    member this.PostPins(body: Pin) =
        async {
            let requestParts = [ RequestPart.jsonContent body ]
            let! (status, content) = OpenApiHttp.postAsync httpClient "/pins" requestParts

            if status = HttpStatusCode.Accepted then
                return PostPins.Accepted(Serializer.deserialize content)
            else if status = HttpStatusCode.BadRequest then
                return PostPins.BadRequest(Serializer.deserialize content)
            else if status = HttpStatusCode.Unauthorized then
                return PostPins.Unauthorized(Serializer.deserialize content)
            else if status = HttpStatusCode.NotFound then
                return PostPins.NotFound(Serializer.deserialize content)
            else
                return PostPins.Conflict(Serializer.deserialize content)
        }

    ///<summary>
    ///Get a pin object and its status
    ///</summary>
    member this.GetPinsByRequestid(requestid: string) =
        async {
            let requestParts =
                [ RequestPart.path ("requestid", requestid) ]

            let! (status, content) = OpenApiHttp.getAsync httpClient "/pins/{requestid}" requestParts

            if status = HttpStatusCode.OK then
                return GetPinsByRequestid.OK(Serializer.deserialize content)
            else if status = HttpStatusCode.BadRequest then
                return GetPinsByRequestid.BadRequest(Serializer.deserialize content)
            else if status = HttpStatusCode.Unauthorized then
                return GetPinsByRequestid.Unauthorized(Serializer.deserialize content)
            else if status = HttpStatusCode.NotFound then
                return GetPinsByRequestid.NotFound(Serializer.deserialize content)
            else
                return GetPinsByRequestid.Conflict(Serializer.deserialize content)
        }
    ...
    ///<param name="body">Pin object</param>
    member this.PostPinsByRequestid(requestid: string, body: Pin) =
        async {
            let requestParts =
                [ RequestPart.path ("requestid", requestid)
                  RequestPart.jsonContent body ]

            let! (status, content) = OpenApiHttp.postAsync httpClient "/pins/{requestid}" requestParts

            if status = HttpStatusCode.Accepted then
                return PostPinsByRequestid.Accepted(Serializer.deserialize content)
            else if status = HttpStatusCode.BadRequest then
                return PostPinsByRequestid.BadRequest(Serializer.deserialize content)
            else if status = HttpStatusCode.Unauthorized then
                return PostPinsByRequestid.Unauthorized(Serializer.deserialize content)
            else if status = HttpStatusCode.NotFound then
                return PostPinsByRequestid.NotFound(Serializer.deserialize content)
            else
                return PostPinsByRequestid.Conflict(Serializer.deserialize content)
        }

    ///<summary>
    ///Remove a pin object
    ///</summary>
    member this.DeletePinsByRequestid(requestid: string) =
        async {
            let requestParts =
                [ RequestPart.path ("requestid", requestid) ]

            let! (status, content) = OpenApiHttp.deleteAsync httpClient "/pins/{requestid}" requestParts

            if status = HttpStatusCode.Accepted || content = "OK" then
                return DeletePinsByRequestid.Accepted
            else if status = HttpStatusCode.BadRequest then
                return DeletePinsByRequestid.BadRequest(Serializer.deserialize content)
            else if status = HttpStatusCode.Unauthorized then
                return DeletePinsByRequestid.Unauthorized(Serializer.deserialize content)
            else if status = HttpStatusCode.NotFound then
                return DeletePinsByRequestid.NotFound(Serializer.deserialize content)
            else
                printfn $"\"{content}\""
                return DeletePinsByRequestid.Conflict(Serializer.deserialize content)
        }
